...
- **`audioread` must stay an explicit dependency** — qwen-omni-utils 0.0.9 imports `audioread` at module scope, but it's no longer pulled in transitively by librosa/soundfile. Removing it from the image breaks the import even though nothing in our code calls it directly.
- **`HF_HUB_ENABLE_HF_TRANSFER` is deprecated and inert** — `HF_XET_HIGH_PERFORMANCE=1` replaces it for fast HF Hub downloads. This is why the old hf_transfer re-install step is gone from the image.
